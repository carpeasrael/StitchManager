...
fn read_u16_be(data: &[u8], offset: usize) -> Result<u16, Fehler> {
    let end = offset
        .checked_add(2)
...
fn read_i16_be(data: &[u8], offset: usize) -> Result<i16, Fehler> {
    let end = offset
        .checked_add(2)
        .ok_or_else(|| parse_err(format!("Offset overflow at {offset}")))?;
    if end > data.len() {
...
fn read_i32_be(data: &[u8], offset: usize) -> Result<i32, Fehler> {
    let end = offset
        .checked_add(4)
        .ok_or_else(|| parse_err(format!("Offset overflow at {offset}")))?;
    if end > data.len() {
...
#[cfg(test)]
fn read_vp3_string(data: &[u8], offset: usize) -> Result<(String, usize), Fehler> {
...
fn parse_vp3_design(data: &[u8], start: usize) -> Result<(Vp3DesignInfo, usize), Fehler> {
    let mut pos = start;
    let mut colors = Vec::new();
    let mut total_stitches: u32 = 0;
    let mut total_jumps: u32 = 0;
    ...
    while pos + 10 <= scan_limit {
        ...
        if let Some((color, stitch_data_start, section_end)) = try_parse_color_section(data, pos) {
            ...
            let (section_stitches, section_jumps) =
                count_vp3_stitches(data, stitch_data_start, section_end);
...
fn try_parse_color_section(data: &[u8], pos: usize) -> Option<(ParsedColor, usize, usize)> {
    ...
    for skip in 0..8 {
        ...
        if after_rgb + 2 <= block_end {
            ...
            if name_len < 100 && after_rgb + 2 + name_len <= block_end {
                let name = String::from_utf8_lossy(&data[after_rgb + 2..after_rgb + 2 + name_len])
                    .to_string();
                ...
                let stitch_start = if let Some(bp2) = brand_pos.checked_add(2) {
                    if bp2 <= block_end {
                        ...
                        let after_brand =
                            bp2.saturating_add(brand_len_val.min(block_end.saturating_sub(bp2)));
...
fn scan_vp3_structure(data: &[u8]) -> Option<Vp3DesignInfo> {
    ...
    while pos + 10 < scan_limit {
        ...
        if !(3..100).contains(&name_len) || pos + 5 + name_len as usize > data.len() {
            consecutive_misses += 1;
            if consecutive_misses > 10_000 {
...
fn decode_vp3_stitch_segments(data: &[u8]) -> Vec<StitchSegment> {
    // SM-FMT-012: keine unbegrenzte Speicherbelegung. Gezählt werden alle
    // Punkte über alle Farblagen; ist die Grenze erreicht, bricht die
    // Auswertung ab und liefert, was bis dahin gelesen wurde.
    let mut punkte_gesamt: usize = 0;
    ...
    while pos + 10 <= stitch_scan_limit {
        if punkte_gesamt > crate::HOECHSTZAHL_PUNKTE {
            break;
        }
        if let Some((color, stitch_start, section_end)) = try_parse_color_section(data, pos) {
            ...
            while sp + 4 <= effective_end {
                ...
                if let (Ok(dx), Ok(dy)) = (read_i16_be(data, sp), read_i16_be(data, sp + 2)) {
                    ...
                    let is_jump = (dx as i32).abs() > VP3_JUMP_THRESHOLD
                        || (dy as i32).abs() > VP3_JUMP_THRESHOLD;
...
mod tests {
    ...
    fn test_scan_vp3_requires_brand_name() {
        ...
        assert!(
            result.is_none(),
            "Missing brand name should cause rejection"
        );
    }
