//! Schreibpfad.
//!
//! Die Ziele sind vom Typ [`kern_security::Schreibziel`] und lassen sich nur
//! über `Wurzel::schreibziel` herstellen. Damit erzwingt der Übersetzer
//! Schnittregel 5 — „jeder Schreibvorgang läuft durch `kern/security`" —
//! statt sie einer Zusage zu überlassen.
use super::StitchSegment;
use kern_security::Schreibziel;
use kern_typen::Fehler;

/// Write a DST file from stitch segments.
/// DST format: 512-byte header + balanced-ternary stitch data.
pub fn write_dst(segments: &[StitchSegment], ziel: &Schreibziel) -> Result<(), Fehler> {
    ...
    if all_stitches.is_empty() {
        return Err(Fehler::Eingabe("Keine Stichdaten vorhanden".into()));
    }
    ...
    write_header_field(&mut header, 0, "LA:Stitch Manager    \r");
    ...
    std::fs::write(ziel.pfad(), &output)
        .map_err(|e| Fehler::aus_ea(e, "Schreiben einer Stickdatei"))?;
    Ok(())
}

/// Write a PES file from stitch segments.
/// Simplified PES v1 format: PES header + PEC block with stitch data.
pub fn write_pes(segments: &[StitchSegment], ziel: &Schreibziel) -> Result<(), Fehler> {
...
fn balanced_ternary(mut value: i32) -> [i8; 5] {
    ...
    for (i, &w) in weights.iter().enumerate() {
        ...
        // 0 is a member of the candidate set, so `min_by_key` over a non-empty
        // total list can't return None. Fall back to 0 to keep this branch
        // panic-free even if the candidate array is ever modified.
        let best = [-1i8, 0, 1]
            .iter()
            .copied()
...
pub fn convert_segments(
    ...
    match target_format.to_uppercase().as_str() {
        "DST" => write_dst(segments, ziel),
        "PES" => write_pes(segments, ziel),
        _ => Err(Fehler::Eingabe(format!(
            "Das Zielformat {target_format} wird nicht unterstützt."
        ))),
    }
}
