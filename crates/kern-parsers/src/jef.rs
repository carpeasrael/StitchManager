...
fn read_i32_le(data: &[u8], offset: usize) -> Result<i32, Fehler> {
    let end = offset
        .checked_add(4)
        .ok_or_else(|| parse_err(format!("Offset overflow at {offset}")))?;
    if end > data.len() {
...
fn read_u32_le(data: &[u8], offset: usize) -> Result<u32, Fehler> {
    let end = offset
        .checked_add(4)
        .ok_or_else(|| parse_err(format!("Offset overflow at {offset}")))?;
    if end > data.len() {
...
fn janome_color(index: u32) -> ParsedColor {
    if let Some(&(_, r, g, b, name)) = JANOME_PALETTE.iter().find(|&&(code, ..)| code == index) {
        ...
        ParsedColor {
            hex: "#808080".to_string(), // D-05-Ausnahme: Garnfarbe der Janome-Palette — Daten aus dem Dateiformat, keine Gestaltung.
...
fn parse_jef_header(data: &[u8]) -> Result<JefHeaderInfo, Fehler> {
    ...
    let matches_compact =
        cc_at_16 > 0 && cc_at_16 <= 256 && stitch_offset == JEF_MIN_HEADER + cc_at_16 as usize * 4;
...
impl EmbroideryParser for JefParser {
    ...
    fn parse(&self, data: &[u8]) -> Result<ParsedFileInfo, Fehler> {
        ...
        let (stitch_count_decoded, jump_count) =
...
fn count_jef_stitches_and_jumps(data: &[u8], start: usize) -> (u32, u32) {
    ...
    while pos < data.len() {
        // SM-FMT-012: keine unbegrenzte Speicherbelegung. Ohne diese Grenze
        // beschreibt eine manipulierte Datei beliebig viele Stiche.
        if count as usize > crate::HOECHSTZAHL_PUNKTE {
            break;
        }
        let b = data[pos];
...
pub(crate) fn decode_jef_stitch_coordinates(
    data: &[u8],
    stitch_offset: usize,
    ...
    let mut punkte: usize = 0;

    current_segment.push((x, y));

    while pos < data.len() {
        // SM-FMT-012: siehe oben.
        punkte += 1;
        if punkte > crate::HOECHSTZAHL_PUNKTE {
            break;
        }
        let b = data[pos];
...
mod tests {
    ...
    fn test_janome_color_known() {
        ...
        assert_eq!(color.hex, "#000000"); // D-05-Ausnahme: Erwartungswert eines Prüffalls über gelesene Garnfarben.
    ...
    fn test_janome_color_unknown() {
        ...
        assert_eq!(color.hex, "#808080"); // D-05-Ausnahme: Erwartungswert eines Prüffalls über gelesene Garnfarben.
    ...
    fn test_janome_color_high_index() {
        ...
        assert_ne!(color.hex, "#808080", "Index 50 should not be gray"); // D-05-Ausnahme: Erwartungswert eines Prüffalls über gelesene Garnfarben.
    ...
    fn test_janome_color_white() {
        ...
        assert_eq!(color.hex, "#FFFFFF"); // D-05-Ausnahme: Erwartungswert eines Prüffalls über gelesene Garnfarben.
    ...
    fn test_parse_synthetic_jef() {
        ...
        assert_eq!(info.colors[0].hex, "#000000"); // Color index 1 = Black  // D-05-Ausnahme: Erwartungswert eines Prüffalls über gelesene Garnfarben.
        assert_eq!(info.colors[1].hex, "#FF0000"); // Color index 5 = Red  // D-05-Ausnahme: Erwartungswert eines Prüffalls über gelesene Garnfarben.
        assert_eq!(info.colors[2].hex, "#1A842D"); // Color index 9 = Green  // D-05-Ausnahme: Erwartungswert eines Prüffalls über gelesene Garnfarben.
    }
    ...
    fn test_jef_stitch_offset_validation() {
        ...
        assert_eq!(info.colors[0].hex, "#000000"); // Black  // D-05-Ausnahme: Erwartungswert eines Prüffalls über gelesene Garnfarben.
        assert_eq!(info.colors[1].hex, "#FF0000"); // Red  // D-05-Ausnahme: Erwartungswert eines Prüffalls über gelesene Garnfarben.
    }
