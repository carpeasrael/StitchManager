...
use kern_typen::Fehler;

pub struct DstParser;

const DST_HEADER_SIZE: usize = 512;

/// Hard cap on decoded stitch triplets to defend against malformed files that
/// omit the End marker (0xF3) and would otherwise iterate over the entire byte
/// stream. 1 M stitches comfortably exceeds any real embroidery design.
const DST_MAX_TRIPLETS: usize = 1_000_000;
...
fn parse_err(msg: impl Into<String>) -> Fehler {
    Fehler::Parser {
...
pub(crate) fn decode_dst_triplet(b0: u8, b1: u8, b2: u8) -> (i32, i32) {
    ...
    let dx = bit(b2, 2) * 81 - bit(b2, 3) * 81 + bit(b1, 2) * 27 - bit(b1, 3) * 27 + bit(b0, 2) * 9
        - bit(b0, 3) * 9
        + bit(b1, 0) * 3
        - bit(b1, 1) * 3
        + bit(b0, 0)
    ...
    let dy = bit(b2, 5) * 81 - bit(b2, 4) * 81 + bit(b1, 5) * 27 - bit(b1, 4) * 27 + bit(b0, 5) * 9
        - bit(b0, 4) * 9
        + bit(b1, 7) * 3
        - bit(b1, 6) * 3
        + bit(b0, 7)
...
pub(crate) fn decode_dst_stitch_segments(data: &[u8]) -> Vec<StitchSegment> {
    ...
    let mut triplet_idx: usize = 0;
    while pos + 3 <= data.len() {
        triplet_idx += 1;
        if triplet_idx > DST_MAX_TRIPLETS {
            // Defensive cap — bail out rather than iterate over a malformed stream
            // (segment decoder cannot return Result, so we surface partial work).
            log::warn!(
                "decode_dst_stitch_segments: triplet cap {DST_MAX_TRIPLETS} reached at pos {pos}, truncating"
            );
            break;
        }
        let b0 = data[pos];
...
impl EmbroideryParser for DstParser {
    ...
    fn parse(&self, data: &[u8]) -> Result<ParsedFileInfo, Fehler> {
        ...
        while pos + 3 <= data.len() {
            triplet_idx += 1;
            if triplet_idx > DST_MAX_TRIPLETS {
                return Err(Fehler::ParserGrenze {
                    was: format!(
                        "DST triplet count exceeded {DST_MAX_TRIPLETS} without End marker (pos={pos})"
                    ),
                });
            }
            let b2 = data[pos + 2];
    ...
    fn extract_thumbnail(&self, _data: &[u8]) -> Result<Option<Vec<u8>>, Fehler> {
        Ok(None)
    }
...
mod tests {
    ...
    fn test_dst_magic_validation() {
        ...
        let result =
            parser.parse(b"NOT_DST_DATA_HERE_NEEDS_TO_BE_512_BYTES_LONG_TO_PASS_SIZE_CHECK");
    ...
    #[test]
    #[ignore = "braucht den externen Prüfbestand (AP-02 Stufe a); nicht im Repository"]
    ...
    fn test_parse_multiple_dst_files() {
        ...
        let files = [
            "2.DST",
            "4.DST",
            "5X7_FollowTheBunnyHeHasChocolate_Fill.dst",
        ];
    ...
    fn test_parse_dst_header_values() {
        ...
        assert!(
            sc > 0 && sc < 27255,
            "Stitch count {sc} should exclude jumps"
        );
    ...
    fn test_dst_stitch_segments() {
        ...
        assert!(
            segments[0].points.len() > 1,
            "First segment should have points"
        );
    }

    #[test]
    fn test_dst_triplet_cap_rejects_unterminated_stream() {
        // Build a header with parseable numeric fields at the exact offsets the
        // parser expects, then append > 1M normal-stitch triplets without an End marker.
        let mut data = vec![b' '; DST_HEADER_SIZE];
        // Required magic at offset 0
        data[..3].copy_from_slice(b"LA:");

        // Numeric fields at fixed offsets (ST/CO/+X/-X/+Y/-Y), each padded to its length.
        let write_num = |buf: &mut [u8], off: usize, len: usize, val: i64| {
            let s = format!("{val:>len$}", len = len);
            buf[off..off + len].copy_from_slice(s.as_bytes());
        };
        write_num(&mut data, ST_VALUE_OFFSET, ST_VALUE_LEN, 1000);
        write_num(&mut data, CO_VALUE_OFFSET, CO_VALUE_LEN, 1);
        write_num(&mut data, PLUS_X_VALUE_OFFSET, EXTENT_VALUE_LEN, 10);
        write_num(&mut data, MINUS_X_VALUE_OFFSET, EXTENT_VALUE_LEN, 10);
        write_num(&mut data, PLUS_Y_VALUE_OFFSET, EXTENT_VALUE_LEN, 10);
        write_num(&mut data, MINUS_Y_VALUE_OFFSET, EXTENT_VALUE_LEN, 10);

        // (DST_MAX_TRIPLETS + 1) normal-stitch triplets (b2 low nibble = 0x03 → Normal).
        let mut payload = vec![0u8; (DST_MAX_TRIPLETS + 1) * 3];
        for i in (2..payload.len()).step_by(3) {
            payload[i] = 0x03;
        }
        ...
        match result {
            Err(Fehler::ParserGrenze { was }) => {
                assert!(was.contains("DST"), "Fehlertext soll DST nennen: {was}");
            }
            other => panic!("expected ParserBounds, got {other:?}"),
        }
    }
}
