...
fn read_u16(data: &[u8], offset: usize) -> Result<u16, Fehler> {
...
fn read_u32(data: &[u8], offset: usize) -> Result<u32, Fehler> {
...
/// Hard cap on decoded PEC stitch records to defend against malformed files
/// that omit the End marker (0xFF). Mirrors `DST_MAX_TRIPLETS` in scope and
/// rationale (see parsers/dst.rs).
const PEC_MAX_STITCHES: usize = 1_000_000;
...
fn decode_pec_stitches(data: &[u8], stitch_start: usize) -> (u32, u16, u32, u32) {
    ...
    while pos < data.len() {
        iter_count += 1;
        if iter_count > PEC_MAX_STITCHES {
            log::warn!(
                "decode_pec_stitches: cap {PEC_MAX_STITCHES} reached at pos {pos} without 0xFF end marker — truncating"
            );
            break;
        }
        let b = data[pos];
...
fn decode_pec_stitch_segments(
    ...
    while pos < data.len() {
        ...
        if iter_count > PEC_MAX_STITCHES {
            log::warn!(
                "decode_pec_stitch_segments: cap {PEC_MAX_STITCHES} reached at pos {pos} — truncating partial segments"
            );
            break;
        }
        let b = data[pos];
...
fn parse_pes_extended_meta(data: &[u8], pec_offset: usize) -> PesExtendedMeta {
    ...
    // Last field — gracefully handle truncation
    let comments = read_bounded(&mut pos).unwrap_or_default();
...
fn parse_pes_header(data: &[u8]) -> Result<PesHeaderInfo, Fehler> {
    if data.len() < 12 {
    ...
    let stitch_start = pec_offset
        .checked_add(532)
        .ok_or_else(|| parse_err("PEC offset overflow when computing stitch start position"))?;
...
impl EmbroideryParser for PesParser {
    ...
    fn parse(&self, data: &[u8]) -> Result<ParsedFileInfo, Fehler> {
        ...
        let design_name = if hdr.name_len > 0 && 17 + hdr.name_len <= data.len() {
            let raw = String::from_utf8_lossy(&data[17..17 + hdr.name_len])
                .trim()
                .to_string();
            if raw.is_empty() {
                None
...
#[cfg(test)]
#[allow(clippy::vec_init_then_push)] // tests interleave byte comments — Vec::new() + push reads clearer than vec![]
mod tests {
    use super::*;
    ...
    fn test_extract_thumbnail_bayrisches_herz() {
        ...
        let has_black = pixels.contains(&0);
        let has_white = pixels.contains(&255);
    ...
    fn test_pec_palette_fallback_for_old_versions() {
        ...
        assert_eq!(info.colors[0].hex, "#ED171F"); // D-05-Ausnahme: Erwartungswert eines Prüffalls über gelesene Garnfarben.
        assert_eq!(info.colors[0].name.as_deref(), Some("Red"));
        assert_eq!(info.colors[1].hex, "#0A55A3"); // D-05-Ausnahme: Erwartungswert eines Prüffalls über gelesene Garnfarben.
    ...
    fn test_jump_vs_trim_detection() {
        ...
        let (stitch_count, _, jump_count, trim_count) = decode_pec_stitches(&stitch_data, 0);
